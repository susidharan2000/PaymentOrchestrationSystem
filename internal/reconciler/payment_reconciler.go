...
func StartPaymentReconciler(repo ReconcilerRepository, r *rand.Rand, registry *psp.Registry) {
	batchSize := 50 // default
	if val := os.Getenv("PAYMENT_RECONCILER_BATCH_SIZE"); val != "" {
	...
	spanPaymentWorkers(repo, PaymentChan, registry)
	for {
...
func spanPaymentWorkers(repo ReconcilerRepository, PaymentChan chan Payment, registry *psp.Registry) {
	workerCount := 1 // default
	if val := os.Getenv("PAYMENT_RECONCILER_CONCURRENCY"); val != "" {
		if parsed, err := strconv.Atoi(val); err == nil && parsed > 0 {
			workerCount = parsed
		}
	}
